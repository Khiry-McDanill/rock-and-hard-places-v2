...
public class PortfolioService {
 ...
 public PortfolioService(PortfolioItemRepository items,PortfolioPublicationRequestRepository publications,TaskAssignmentRepository assignments,TaskProgressService progress){this.items=items;this.publications=publications;this.assignments=assignments;this.progress=progress;}
 ...
 public PortfolioPublicationRequest cancel(Tradesperson actor,PortfolioPublicationRequest request){if(!same(actor,request.getPortfolioItem().getTradesperson()))throw new IllegalArgumentException("Only the portfolio owner may cancel publication");request.cancel();return publications.save(request);}
