...
public class ReviewService {
 private final ReviewRepository reviews; private final ReviewResponseRepository responses; private final TaskAssignmentRepository assignments; private final TaskProgressService progress;
 public ReviewService(ReviewRepository reviews,ReviewResponseRepository responses,TaskAssignmentRepository assignments,TaskProgressService progress){this.reviews=reviews;this.responses=responses;this.assignments=assignments;this.progress=progress;}
 public Review createTaskReview(Homeowner author,Tradesperson subject,Task task,int overall,Integer quality,Integer communication,Integer reliability,Integer professionalism,String body){
  requireOwner(author,task.getProject()); requireNotSelf(author,subject);
 ...
 public Review createProjectReview(Homeowner author,Tradesperson subject,Project project,int overall,Integer quality,Integer communication,Integer reliability,Integer professionalism,String body){
  requireOwner(author,project);requireNotSelf(author,subject);
 ...
 public Review edit(Homeowner actor,Review review,int overall,Integer quality,Integer communication,Integer reliability,Integer professionalism,String body){requireAuthor(actor,review);review.edit(overall,quality,communication,reliability,professionalism,body);return reviews.save(review);}
 public Review withdraw(Homeowner actor,Review review){requireAuthor(actor,review);review.withdraw();return reviews.save(review);}
 public ReviewResponse respond(Tradesperson actor,Review review,String body){requireSubject(actor,review);if(responses.findByReview(review).isPresent())throw new IllegalStateException("Review already has a response");return responses.save(new ReviewResponse(review,actor,body));}
